...
const PostJobReviewHistoryJobCard: React.FC<{
    data: Job;
    setPostJob:any
}> = ({data, setPostJob}) => {
    const navigation = useNavigation<NavigationProp<RootStackParamList, any>>();
    async function openPostJobHandler() {
        try {
            const res = await fetch(
                `${API_URL}/job/open-job/${data.job_id}`,
                {
                    method: 'put',
                    headers: {
            ...
            if (res.ok) {
                ...
                setPostJob((prev:any)=>{
                    return prev.filter((obj:any)=>obj.job_id !== data.job_id)
                })
            } else {
...
export default PostJobReviewHistoryJobCard;
